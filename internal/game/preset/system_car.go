...
func NewCarSystem(scene *Scene) *CarSystem {
	return &CarSystem{
		ecsScene: scene.ECS(),
		gfxScene: scene.Graphics(),
...
func (s *CarSystem) OnFixedUpdate(elapsedSeconds float64) {
	s.mouseOfInterest = false

	result := s.ecsScene.Query(
		ecs.IsHealthy(),
		s.ecsScene.HasCarComponent(),
	)
	defer result.Release()

	result.Each(func(entity Entity) {
		keyboardControl := entity.RefCarKeyboardComponent()
		if keyboardControl != nil {
			s.updateKeyboard(elapsedSeconds, entity)
		}

		mouseControl := entity.RefCarMouseComponent()
		if mouseControl != nil {
			s.mouseOfInterest = true
			s.updateMouse(elapsedSeconds, entity)
		}

		gamepadControl := entity.RefCarGamepadComponent()
		if gamepadControl != nil {
...
func (s *CarSystem) updateKeyboard(elapsedSeconds float64, entity Entity) {
	carComp := entity.RefCarComponent()
	keyboardComp := entity.RefCarKeyboardComponent()
...
func (s *CarSystem) updateMouse(elapsedSeconds float64, entity Entity) {
	carComp := entity.RefCarComponent()
	mouseComp := entity.RefCarMouseComponent()
...
func (s *CarSystem) updateGamepad(elapsedSeconds float64, entity Entity) {
	carComp := entity.RefCarComponent()
	gamepadComp := entity.RefCarGamepadComponent()
...
func (s *CarSystem) updateCar(elapsedSeconds float64, entity Entity) {
	...
	for _, light := range carComp.Car.Chassis().HeadLights() {
		light.SetVisible(carComp.LightsOn)
	}
	...
	for _, light := range carComp.Car.Chassis().StopLights() {
		light.SetVisible(carComp.Deceleration > 0.1)
	}
