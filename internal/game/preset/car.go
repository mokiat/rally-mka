...
func (d *CarDefinition) ApplyToModel(scene *Scene, info CarApplyInfo) *Car {
	...
	scene.BodyBindingSet().Bind(chassisNode, chassisBody)

	headLights := make([]hierarchy.Node, len(d.chassisDef.headLightNodeNames))
	for i, nodeName := range d.chassisDef.headLightNodeNames {
		node := info.Model.FindNode(nodeName)
		headLights[i] = scene.Hierarchy().Wrap(node)
	}

	tailLights := make([]hierarchy.Node, len(d.chassisDef.tailLightNodeNames))
	for i, nodeName := range d.chassisDef.tailLightNodeNames {
		node := info.Model.FindNode(nodeName)
		tailLights[i] = scene.Hierarchy().Wrap(node)
	}

	beamLights := make([]hierarchy.Node, len(d.chassisDef.beamLightNodeNames))
	for i, nodeName := range d.chassisDef.beamLightNodeNames {
		node := info.Model.FindNode(nodeName)
		beamLights[i] = scene.Hierarchy().Wrap(node)
	}

	stopLights := make([]hierarchy.Node, len(d.chassisDef.stopLightNodeNames))
	for i, nodeName := range d.chassisDef.stopLightNodeNames {
		node := info.Model.FindNode(nodeName)
		stopLights[i] = scene.Hierarchy().Wrap(node)
	}
	...
	for _, axisDef := range d.axesDef {
		...
		scene.BodyBindingSet().Bind(leftWheelNode, leftWheelBody)
		...
		if hubDef := axisDef.leftHubDef; hubDef != nil {
			...
			scene.BodyBindingSet().Bind(hubNode, hubBody)
		...
		scene.BodyBindingSet().Bind(rightWheelNode, rightWheelBody)
	...
	entity.SetNodeComponent(NodeComponent{
		Node: scene.Hierarchy().Wrap(chassisNode),
	})
	result := &Car{
		chassis: &Chassis{
			node:       chassisNode,
	...
	entity.SetCarComponent(CarComponent{
		Car:            result,
		Gear:           CarGearForward,
		SteeringAmount: 0.0,
		Acceleration:   0.0,
...
type Chassis struct {
	...
	body       physics.Body
	headLights []hierarchy.Node
	tailLights []hierarchy.Node
	beamLights []hierarchy.Node
	stopLights []hierarchy.Node
}

func (c *Chassis) Node() hierarchy.NodeID {
	return c.node
}
...
func (c *Chassis) HeadLights() []hierarchy.Node {
	return c.headLights
}

func (c *Chassis) TailLights() []hierarchy.Node {
	return c.tailLights
}

func (c *Chassis) BeamLights() []hierarchy.Node {
	return c.beamLights
}

func (c *Chassis) StopLights() []hierarchy.Node {
	return c.stopLights
}
...
func (w *Wheel) Node() hierarchy.NodeID {
	return w.node
}
...
func (h *Hub) Node() hierarchy.NodeID {
	return h.node
}
