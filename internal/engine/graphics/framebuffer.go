...
func (b *Framebuffer) Allocate(data FramebufferData) error {
	...
	if data.HasAlbedoAttachment {
		...
		if b.AlbedoTextureID == 0 {
			return fmt.Errorf("failed to allocate albedo texture attachment")
		}
	...
	if data.HasNormalAttachment {
		...
		if b.NormalTextureID == 0 {
			return fmt.Errorf("failed to allocate normal texture attachment")
		}
	...
	if data.HasDepthAttachment {
		...
		if b.DepthTextureID == 0 {
			return fmt.Errorf("failed to allocate depth texture attachment")
		}
